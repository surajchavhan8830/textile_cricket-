...
class MatchInformationController extends Controller
{
    ...
    public function add_match_over(Request $request)
    {
        ...
        // $change_strick = MatchOver::latest()->first();
        // $change_strick->sticker_player_id = 
        // $running_over = ($change_strick->over_number-1).'.'.$change_strick->ball_number;
        ...
        // $count_run = $request->run;

        // $overs = MatchOver::select('bowler_player_id', );
        ...
        if ($request->betting_team_id == $match_info->team_1) {
            ...
            // $match_info_update->team_2_total_over = $request->over_number;
        }
        ...
        $six  = ($request->run == 6) ? 1 : 0;
        $four = ($request->run == 4) ? 1 : 0;
        ...
        // if ($request->ball_type == "wb") {
        //     $matchBatsman->run = $matchBatsman->run;
        // } elseif ($request->ball_type == "nb" && $request->no_ball_type == "bat") {
        //     $matchBatsman->run = $matchBatsman->run +  $request->run;
        // } elseif ($request->ball_type == "nb" && $request->no_ball_type == "byes" || $request->ball_type == "nb" && $request->no_ball_type == "legbyes") {
        //     $matchBatsman->run = $matchBatsman->run;
        // } elseif($request->ball_type == "by" && $request->ball_type == "lb")
        // {
        //     $matchBatsman->run = $matchBatsman->run;
        // }
        //  else {
        //     $matchBatsman->run = $matchBatsman->run + (int) $request->run;
        // }

        // if ($is_normal_ball_delivery) {
        //     $matchBatsman->balls = $matchBatsman->balls + 1;    // TODO: CHECK Normal Ball Only
        //     $matchBatsman->sixers = $matchBatsman->sixers + $six;
        //     $matchBatsman->fours = $matchBatsman->fours + $four;
        // } elseif ($request->ball_type == "nb" && $request->no_ball_type == "bat") {
        //     $matchBatsman->balls = $matchBatsman->balls + 1;
        // } elseif ($request->ball_type == "nb" && $request->no_ball_type == "byes") {
        //     $matchBatsman->balls = $matchBatsman->balls + 1;
        // } elseif ($request->ball_type == "nb" && $request->no_ball_type == "legbyes") {
        //     $matchBatsman->balls = $matchBatsman->balls + 1;
        // } elseif ($request->ball_type == "by" || $request->ball_type == "lb") {
        //     $matchBatsman->balls = $matchBatsman->balls + 1;
        // } else {

        //     $matchBatsman->balls = $matchBatsman->balls;
        // }

        // --- INCASE OF OUT ONLY
        // $matchBatsman->type_out = $request->out_type;
        // $matchBatsman->out_by_player_id = $request->out_by_player_id;
        // $matchBatsman->out_by_bowler_id = $request->bowler_player_id;
        // // --- INCASE OF OUT ONLY
        $matchBatsman->save();
        // dd($matchBatsman); 
        // --- --- --- UPDATE BETSMAN SUMMARY 
        ...
        // $wicket_player = $over_match->
        // dd(MatchOver::select('over_number', DB::raw('SUM(run) as total'))
        ...
        // if ($request->ball_type == "nb" && $request->no_ball_type == "bat") {
        //     $matchBowler->runs  = $over_by_run + $over_by_extra;
        // } elseif ($request->ball_type == "nb" && $request->no_ball_type == "lb") {
        //     $matchBowler->runs  = $over_by_run;
        // } elseif ($request->ball_type == "nb" && $request->no_ball_type == "by" || $request->ball_type == "by" || $request->ball_type == "lb") {
        //     $matchBowler->runs  = $over_by_run;
        // } elseif ($request->ball_type == "nb" && $request->no_ball_type == "byes" || $request->ball_type == "nb" && $request->no_ball_type == "legbyes") {
        //     $matchBowler->runs  = $over_by_run;
        // } elseif ($request->ball_type == "by" || $request->ball_type == "lb") {
    ...
    public function matchinfo(Request $request, $id)
    {
        ...
        /* ------------------------------------------ */
        //$current_over_total_normal_balls = MatchOver::where('over_number', $current_over)->where('ball_type', 'normal')->where('match_ids', $match_id)->count();
        ...
        if ($matchinfo->runing_over && ($matchinfo->runing_over - (int)$matchinfo->runing_over == 0)) {
            ...
            //                 ->get();
            // if ($extra_balls->sum('is_extra') > 0 && ($matchinfo->runing_over - (int)$matchinfo->runing_over > 0)) {
            //     $is_new_over = false;
            // }
            // dd($is_new_over);
        ...
        // ----------- OLD
        // $balls = MatchOver::select('run', 'ball_type')->where('match_ids', $match_id)
        ...
        // TODO: NEED TO SEND IN DIFFERENT VARIABLES
        $matchinfo->team_1_total_run = $matchinfo->team_1_total_run + $matchinfo->team_1_extra_run;
        $matchinfo->team_2_total_run = $matchinfo->team_2_total_run + $matchinfo->team_2_extra_run;
        // TODO: NEED TO SEND IN DIFFERENT VARIABLES
        ...
        $partnership_ball_list = MatchOver::select('ball_number',  'team_id', 'run', 'ball_type', 'out_type', 'is_extra', 'is_normal')
            // ->where('over_number', '=', ($is_new_over ? (int) $running_over - 1 : (int) $running_over))
        ...
        $partnership_run = $partnership_ball_list->sum('run');
        $partnership_ball = $partnership_ball_list->sum('is_normal');
    ...
    public function scorecard(Request $request, $id)
    {
        ...
        //  dd($matchinfo->nonstickerScore->out_by_player_id);
        // $running_over = $matchinfo->runing_over;
        $bowler_id = $matchinfo->bowler_id;
    ...
    public function matchResult(Request $request)
    {
        ...
        $match_info = MatchInformation::with('team1', 'team2')->where('id', $match_id)->first();

        if ($match_info->inning_id == 2) {

            // CHECK WICKET

            // CHECK RUN
            if ($match_info->team_1_total_run > $match_info->team_2_total_run) {
                $result = $match_info->team1->team_name . " WON BY " . $match_info->team_1_total_run - $match_info->team_2_total_run . " RUN";
            } elseif ($match_info->team_2_total_run > $match_info->team_1_total_run) {

                $result = $match_info->team2->team_name . " WON BY " . $match_info->team_2_total_run - $match_info->team_1_total_run . " RUN";
            } else {
                if ($match_info->team_2_total_run ==  $match_info->team_1_total_run) {
                    $result = "Match Drawn";
                }
            }

            $match_info->summary = $result;
            $match_info->inning_id = 3;
            $match_info->save();
        ...
        // if($match_info->team_1 > $match_info->team_)
        // {
