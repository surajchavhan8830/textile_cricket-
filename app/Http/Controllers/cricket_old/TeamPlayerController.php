...
class TeamPlayerController extends Controller
{
    ...
    public function team_wise_player(Request $request)
    {
        ...
        $tournament_id = $request->tournament_id;
        $teamplayer = TeamPlayer::where('team_id', $team_id)->where('postion', "!=", 5 )
        ->orderBy('sequence')->orderBy('id')
        ->with('team','player','playerrole')->get();
    ...
    public function add_player(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'player_id' => Rule::unique('team_player')->where(function ($query) use ($team_id, $tournament_id) {
                return $query->where('team_id', $team_id)->where('tournament_id', $tournament_id);
            }),
        ]);
